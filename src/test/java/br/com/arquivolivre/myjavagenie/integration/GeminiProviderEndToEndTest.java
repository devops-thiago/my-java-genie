...
/**
 * End-to-end integration test for Gemini provider. Tests query flow with Gemini, token usage
 * tracking, error handling, and retries. Tests Requirements: 10.1, 10.4, 10.5
 */
...
class GeminiProviderEndToEndTest {
  ...
  /**
   * Test Requirement 10.1: Test query flow with Gemini Verifies end-to-end query processing using
  ...
  void testErrorHandlingWithGemini() {
    ...
    ResponseEntity<QueryResponse> response1 =
        restTemplate.postForEntity("/api/query", request1, QueryResponse.class);

    // Should handle gracefully
    assertThat(response1.getStatusCode())
        .isIn(HttpStatus.OK, HttpStatus.SERVICE_UNAVAILABLE, HttpStatus.TOO_MANY_REQUESTS);

    // 2. Safety filter error
    QueryRequest request2 = new QueryRequest("Test safety filter");
    ResponseEntity<QueryResponse> response2 =
        restTemplate.postForEntity("/api/query", request2, QueryResponse.class);

    assertThat(response2.getStatusCode())
        .isIn(HttpStatus.OK, HttpStatus.SERVICE_UNAVAILABLE, HttpStatus.BAD_REQUEST);
  }
  ...
  void testTimeoutErrorHandling() {
    ...
    assertThat(response.getStatusCode())
        .isIn(HttpStatus.OK, HttpStatus.SERVICE_UNAVAILABLE, HttpStatus.GATEWAY_TIMEOUT);
