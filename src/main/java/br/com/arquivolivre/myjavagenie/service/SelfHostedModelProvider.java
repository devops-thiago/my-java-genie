...
/**
 * Language model provider for self-hosted models using Ollama.
 * Implements retry logic and error handling for connection failures.
 */
public class SelfHostedModelProvider implements LanguageModelProvider {
    ...
    public SelfHostedModelProvider(ModelConfig config) {
        ...
        this.timeoutSeconds = settings.getTimeoutSeconds() != null ?
                settings.getTimeoutSeconds() : 60;

        logger.info("Initializing self-hosted model provider: {} at {}",
                modelName, settings.getBaseUrl());
