...
/**
 * Implementation of DocumentProcessor that splits text recursively at natural boundaries. Attempts
 * to split at paragraphs first, then sentences, then words, and finally characters.
 */
@Service
public class RecursiveCharacterSplitter implements DocumentProcessor {

  // Separators in order of preference (paragraph, double newline, newline, sentence, space,
  // character)
  ...
  public List<DocumentChunk> chunkText(String text, DocumentMetadata metadata) {
    ...
    for (int i = 0; i < textChunks.size(); i++) {
      ...
      DocumentMetadata chunkMetadata =
          new DocumentMetadata(metadata.getSourceFile(), metadata.getSection(), i);

      // Copy additional properties
      if (metadata.getAdditionalProperties() != null) {
        chunkMetadata.setAdditionalProperties(
            new java.util.HashMap<>(metadata.getAdditionalProperties()));
      }
  ...
  private List<String> mergeSplits(
      List<String> splits, int chunkSize, int chunkOverlap, String separator) {
    List<String> chunks = new ArrayList<>();
  ...
  /**
   * Estimate token count using a simple heuristic. Approximation: 1 token ≈ 4 characters for
   * English text.
   */
