 ...
 * Provides system health status including availability of language model and vector database.
 */
@RestController
@RequestMapping("/api")
public class HealthController {
    ...
    public HealthController(LanguageModelProvider languageModel,
    ...
    public ResponseEntity<HealthResponse> health() {
        ...
        logger.info("Health check completed: status={}, languageModel={}, vectorDb={}",
                response.getStatus(),
        ...
        try {
            ...
            if (available) {
                return new ComponentHealth(
                        ...
                        true,
                        Map.of(
                                "provider", languageModel.getProviderName(),
                                "message", "Language model is available"
                        )
                );
            } else {
                return new ComponentHealth(
                        ...
                        Map.of(
                                "provider", languageModel.getProviderName(),
                                "message", "Language model is not available"
                        )
                );
            }
        } catch (Exception e) {
            ...
            return new ComponentHealth(
                    ...
                    Map.of(
                            "provider", languageModel.getProviderName(),
                            "message", "Error checking language model: " + e.getMessage()
                    )
            );
        }
    }
        ...
        try {
            ...
            if (exists) {
                return new ComponentHealth(
                        ...
                        true,
                        Map.of(
                                "type", vectorDbConfig.getType(),
                                "collection", collectionName,
                                "message", "Vector database is available and collection exists"
                        )
                );
            } else {
                return new ComponentHealth(
            ...
            return new ComponentHealth(
                    ...
                    Map.of(
                            "type", vectorDbConfig.getType(),
                            "message", "Error checking vector database: " + e.getMessage()
                    )
            );
        }
    }
