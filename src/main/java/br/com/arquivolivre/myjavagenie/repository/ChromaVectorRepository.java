...
public class ChromaVectorRepository implements VectorRepository {
  ...
  public void storeBatch(List<DocumentChunk> chunks, List<float[]> embeddings) {
    ...
    if (chunks.size() != embeddings.size()) {
      throw new IllegalArgumentException(
          String.format(
              "Chunks size (%d) must match embeddings size (%d)",
    ...
    executeWithRetry(
        () -> {
          List<TextSegment> segments =
              chunks.stream().map(this::createTextSegment).collect(Collectors.toList());

          List<Embedding> embeddingObjs =
  ...
  private ScoredDocument convertToScoredDocument(EmbeddingMatch<TextSegment> match) {
    ...
    int tokenCount =
        metadata.getInteger("tokenCount") != null ? metadata.getInteger("tokenCount") : 0;
    ...
    for (String key : metadata.toMap().keySet()) {
      if (!key.equals("id")
          && !key.equals("tokenCount")
          && !key.equals("sourceFile")
          && !key.equals("section")
          && !key.equals("chunkIndex")) {
