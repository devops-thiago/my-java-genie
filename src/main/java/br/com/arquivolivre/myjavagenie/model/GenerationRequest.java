...
/** Request model for language model generation. Contains the prompt and generation parameters. */
public class GenerationRequest {
  ...
  public GenerationRequest(
      String prompt, double temperature, int maxTokens, List<String> stopSequences) {
    this.prompt = prompt;
    this.temperature = temperature;
    this.maxTokens = maxTokens;
  ...
  public String toString() {
    return "GenerationRequest{"
        + "prompt='"
        + (prompt != null && prompt.length() > 100 ? prompt.substring(0, 100) + "..." : prompt)
        + '\''
        + ", temperature="
        + temperature
        + ", maxTokens="
